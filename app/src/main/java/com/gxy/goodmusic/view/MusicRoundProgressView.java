package com.gxy.goodmusic.view;
...
public class MusicRoundProgressView extends View {
    ...
    public MusicRoundProgressView(Context context, @Nullable AttributeSet attrs) {
        ...
        // 颜色
        mPaintColor = typedArray.getColor(R.styleable.RoundProgressView_strokeColor, 0xFFFFFF);
        // 圆环半径 = 半径 + 圆环宽度的1/2
        mRingRadius = mRadius + mStrokeWidth / 2;

        mPaint = new Paint();
        mPaint.setAntiAlias(true); // 去掉锯齿
        mPaint.setStyle(Paint.Style.STROKE); //画笔样式为空心，也可以理解为描边
        mPaint.setStrokeWidth(mStrokeWidth); // 描边的宽度
